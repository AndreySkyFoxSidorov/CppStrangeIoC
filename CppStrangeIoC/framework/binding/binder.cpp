...
IBinding* Binder::GetBinding(void* key, void* name)
{
	...
	if(bindings.find(key) != bindings.end())
	{
		...
		name = (name == nullptr) ? BindingConst::NULLOID : name;
...
void Binder::Unbind(void* key, void* name)
{
	...
	{
		std::unordered_map<void*, IBinding*> dict = bindings[key];
		auto bindingName = (name == nullptr) ? BindingConst::NULLOID : name;
...
void Binder::RemoveValue(IBinding* binding, void* value)
{
	...
	{
		dict = bindings [key];
		if(dict.find(binding->name) != dict.end())
		{
			IBinding* useBinding = dict [binding->name];
			useBinding->RemoveValue(value);
			//If result is empty, clean it out
			std::vector<void*> values = dynamic_cast<std::vector<void*>>(useBinding->value);
			if(values.empty() || values.empty())
			{
				dict.erase(useBinding->name);
			}
		}
	}
}
...
void Binder::RemoveKey(IBinding* binding, void* key)
{
	...
	if(dict.find(binding->name) != dict.end())
	{
		...
		std::vector<void*> keys = dynamic_cast<std::vector<void*>>(useBinding->key);
		if(keys.size() > 0 && keys.empty())
		{
			dict.erase(binding->name);
		}
	}
}
...
void Binder::RemoveName(IBinding* binding, void* name)
{
	...
	{
		std::vector<void*> keys = dynamic_cast<std::vector<void*>>(binding->key);
		key = keys [0];
	}
	std::unordered_map<void*, IBinding*> dict = bindings[key];
	if(dict.find(name) != dict.end())
...
void Binder::ResolveBinding(IBinding* binding, void* key)
{
	...
	auto bindingName = (binding->name == nullptr) ? BindingConst::NULLOID : binding->name;
	std::unordered_map<void*, IBinding*> dict;
	if((bindings.find(key) != bindings.end()))
	{
		dict = bindings [key];
		...
		if(dict.find(bindingName) != dict.end())
		{
			...
			if(existingBinding->isWeak)
			{
				...
				if(existingBinding != binding && existingBinding->isWeak && (!binding->isWeak || existingBinding->value == nullptr || dynamic_cast<std::type_info>(existingBinding->value) != nullptr))
				{
...
bool Binder::isConflictCleared(std::unordered_map<IBinding*, void*>& dict, IBinding* binding)
{
	for(auto kv : dict)
	{
		if(kv.first != binding && kv.first->name == binding->name)
		{
			return false;
		}
	}
